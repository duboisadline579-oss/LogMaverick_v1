...
namespace LogMaverick.Views {
    ...
    public partial class LogDetailWindow : Window {
        ...
        private void ParseAndDisplay(string message) {
            ...
            try {
                if (jsonStart >= 0 && (xmlStart < 0 || jsonStart <= xmlStart)) {
                    ...
                    else if (token is JArray arr) _allItems.Add(new KvItem { Key = "array", Value = arr.ToString(Newtonsoft.Json.Formatting.Indented), KeyColor = "#00BFFF" });
                } else if (xmlStart >= 0) {
                    ...
                    if (pretty != null) { var doc = new XmlDocument(); doc.LoadXml(xmlPart); ParseXmlAttributes(doc.DocumentElement, ""); }
        ...
        private int FindXmlStart(string s) {
            int idx = s.IndexOf('<');
            if (idx < 0) return -1;
            if (idx + 1 < s.Length && (s[idx+1] == '?' || s[idx+1] == '!' || char.IsLetter(s[idx+1]))) return idx;
            return -1;
        }
            ...
            foreach (var p in obj.Properties()) {
                ...
                if (p.Value is JObject nested) { ParseJsonObject(nested, key); }
                    ...
                    for (int i = 0; i < arr.Count; i++) {
                        ...
                        else _allItems.Add(new KvItem { Key = $"{key}[{i}]", Value = arr[i].ToString(), KeyColor = kColor });
                    }
                } else {
                    string val = p.Value.ToString();
                    string vColor = val.ToUpper().Contains("ERROR") || val.ToUpper().Contains("FAIL") ? "#FF6B6B" : val.ToUpper().Contains("OK") || val.ToUpper().Contains("SUCCESS") ? "#00C853" : "#DDDDDD";
                    _allItems.Add(new KvItem { Key = key, Value = val, KeyColor = kColor, ValueColor = vColor });
                }
            }
        }
        private void ParseXmlAttributes(XmlNode node, string prefix) {
            if (node == null) return;
            ...
            foreach (XmlNode child in node.ChildNodes)
                if (child.NodeType == XmlNodeType.Element) ParseXmlAttributes(child, key);
        }
